...
Embedding models give you the right substrate (semantic distance instead of token overlap), but a 1536-dimensional cosine score is hard to reason with. You can ask "what is the single best match for this query"; you cannot easily ask "what sits between these two concepts", "what is near this idea but not an exact match", or "what region of the corpus has nothing in it". And you cannot show an operator where in the corpus a query landed. The pipeline's job is to keep the accuracy of the high-dimensional space where it matters and shed dimensions everywhere else.
...
3. Queries that cosine similarity cannot easily express. Once a query has a 3D position, ordinary questions become ordinary queries. Find items near a concept but not an exact match (a radius around the query's position, not its top-k). Find items that sit between two concepts (interpolate the midpoint, return everything in that band). Find gaps in coverage (regions of the map with no points are interesting on their own; they are the things your corpus does not yet know about). Rank candidates by a weighted combination of semantic position and some other attribute (recency, author, cost) in one query rather than two separate scores you have to merge. None of these compose cleanly in 1536D: a "radius" in cosine space has no stable meaning across corpora (0.3 similarity is a different bar in every dataset), a midpoint vector does not interpolate the way human intuition expects, and "empty regions" are unobservable in a space you cannot see.
